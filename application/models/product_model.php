...
class Product_model extends MY_Model
{
    ...
    public function get_product_recommend(){
         $data=$this->db->select('a.*,min(b.price) price,min(b.t_price) t_price')->from('product a')
...
//
    ...
    public function get_product_info($id){
       $info = $this->db->select('a.*,min(b.price) price,min(b.t_price) t_price')->from('product a')
           ->join('product_detail b','a.id = b.pid','left')
           ->where(array(
               'a.id'=>$id
           ))
    ...
    public function confirm_order($order_num){
        $this->db->where('num',$order_num)->where('openid',$this->session->userdata('openid'));
        $rs = $this->db->update('order',array('status'=>2));
        $order_detail = $this->db->select('b.pid,b.qty')->from('order a')
            ->join('order_detail b','b.oid = a.id','left')
            ->where('a.num',$order_num)->where('a.openid',$this->session->userdata('openid'))
            ->get()->result_array();

        if($order_detail){
            foreach ($order_detail as $item) {
                $this->db->set('buy_num', "buy_num+{$item['qty']}", FALSE);
                $this->db->where('id',$item['pid']);
